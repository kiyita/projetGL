...
func _ready():
	Global.turnmode_change.connect(changing_turn_mode)

func changing_turn_mode():
	if (turn_mode == TurnMode.DEFAULT) :
...
func physics_movement(delta: float, player_body: XRToolsPlayerBody, _disabled: bool):
	...
	player_body.rotate_player(deg_to_rad(Global.angle) * sign(left_right))
